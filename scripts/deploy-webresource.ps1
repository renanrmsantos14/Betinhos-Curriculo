param(
  ...
  [string] $SearchText = "TelaCurriculo",

  [string] $FilePath = "dist/new_TelaCurriculo.html",
...
if ([string]::IsNullOrWhiteSpace($WebResourceName)) {
  ...
  if ($matches.Count -eq 0) {
    throw "Nenhum HTML WebResource encontrado contendo '$SearchText'. Rode com -WebResourceName 'nome_unico'."
  }
